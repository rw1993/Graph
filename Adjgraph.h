...
void AdjGraph<T>::BFS1(int k,bool* visited)
{
    ...
    {
        q->DeQueue(i);
        p=vexlist[i]->firstedge;
        if(p) p=p->next;
        while(p)
        {
            if(!visited[p->adjvex])
            {
                cout<<vexlist[p->adjvex]->data<<endl;
...
template<typename T>
void AdjGraph<T>::DFS1(int i,int &count,bool visited[],int dfn[])
{
    EdgeNode*p=new EdgeNode;
    cout<<vexlist[i]->data<<endl;//访问节点
    visited[i]=true;
    dfn[i]=++count;
    p=vexlist[i]->firstedge;
    if(p) p=p->next;//vexlist[i]->firstedge中是不存放adjvex的
    while(p)
    {
        if(!visited[p->adjvex])
        {
            DFS1(p->adjvex,count,visited,dfn);
        }
        p=p->next;
    }
};
template<typename T>
void AdjGraph<T>::DFSTraverse()
{
    bool visited[number_of_edges];
    int  dfn[number_of_nodes];
    int count=0;
    for(int i=0;i<number_of_nodes;i++)
    {
        visited[i]=false;
